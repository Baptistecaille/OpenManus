...
async def run_task(task_id: str, prompt: str):
    try:
        ...
        # Use existing Manus agent with project config
...
async def check_config_status():
    ...
    try:
        current_config = {
            "llm": {
                "default": {
                    "model": config.llm.get("default", {}).get("model", ""),
                    "base_url": config.llm.get("default", {}).get("base_url", ""),
                    "api_key": "***" if config.llm.get("default", {}).get("api_key", "") else "",
                    "max_tokens": config.llm.get("default", {}).get("max_tokens", 4096),
                    "temperature": config.llm.get("default", {}).get("temperature", 0.0),
                }
            }
        }
    ...
    try:
        ...
        if "llm" in config_data:
            if "llm" not in existing_config:
                existing_config["llm"] = {}
            existing_config["llm"]["default"] = config_data["llm"]

        if "server" in config_data:
            if "server" not in existing_config:
                existing_config["server"] = {}
            existing_config["server"] = config_data["server"]

        # Write updated config
        with open(config_path, "w", encoding="utf-8") as f:
            tomllib.dump(existing_config, f)
...
def load_config():
    ...
    try:
        # Try to get server config from existing config
...
if __name__ == "__main__":
    ...
    config = load_config()
    open_with_config = partial(open_local_browser, config["host"], config["port"])

...
def get_daytona_client():
    """Get or create the Daytona client lazily."""
    global _daytona_client
    if _daytona_client is None:
        if not daytona_settings.daytona_api_key:
            raise RuntimeError(
                "Daytona API key is required. Please set 'daytona_api_key' in config/config.toml under [daytona] section."
            )
...
daytona = None  # Will be set lazily when needed
...
async def get_or_start_sandbox(sandbox_id: str):
    ...
    try:
        sandbox = get_daytona_client().get(sandbox_id)
        ...
        if (
            ...
            try:
                get_daytona_client().start(sandbox)
...
def create_sandbox(password: str, project_id: str = None):
    ...
    # Create the sandbox
    sandbox = get_daytona_client().create(params)
...
async def delete_sandbox(sandbox_id: str):
    ...
    try:
        ...
        # Delete the sandbox
        get_daytona_client().delete(sandbox)
